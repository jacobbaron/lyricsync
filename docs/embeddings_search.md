...
A cross-project surface now exists: `GET /api/search?mode=semantic` (SEARCH S3,
#120) reuses this same table/model via `search_clip_embeddings_global`, a
project-agnostic sibling of `search_clip_embeddings` scoped by the caller's RLS
instead of a project id — see `docs/cross_project_search.md`. Coverage is
