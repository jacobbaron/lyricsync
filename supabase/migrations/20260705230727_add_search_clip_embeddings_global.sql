...
-- per-row lookup.
--
-- NOTE: this migration was pre-applied to prod directly during S3 development
-- (schema_migrations.version reconciled to this filename's timestamp,
-- 20260705230727, per CLAUDE.md's documented pre-apply/reconcile pattern) —
-- included here unchanged so `db push` sees no drift and this consolidated
-- branch's migration history matches what's already live.
