...
--
-- Consolidation note (search-consolidated-s2-s3-s6-s7): `snippet` is produced
-- by `ts_headline(...)` with `StartSel=**, StopSel=**` rather than the raw
-- source text, so matched terms already arrive **bold-marked** (stemmed
-- matches included, e.g. a query for "insulate" bolds "insulation") straight
-- from the ranking function. This supersedes S6 (#123)'s JS-side regex
-- marker highlighting for the keyword/FTS path — see
-- docs/cross_project_search.md's "Result enrichment" section for the
-- rationale (native ts_headline understands stemming/tsquery structure that a
-- post-hoc JS regex over raw terms cannot).
...
--
-- `snippet` is `ts_headline(...)` over the raw source text (not the
-- tsvector), with `StartSel=**, StopSel=**` so matched terms arrive already
-- bold-marked (see consolidation note in the file header) — the caller
-- doesn't need to re-highlight.
...
as $$
  ...
  transcript_hits as (
    select
      ...
      csd.timestamp,
      ts_headline('english', coalesce(csd.source_text, ''), q.tsq, 'StartSel=**, StopSel=**') as snippet,
  ...
  description_hits as (
    select
      ...
      ts_headline('english', coalesce(c.visual_description, ''), q.tsq, 'StartSel=**, StopSel=**') as snippet,
  ...
  highlight_hits as (
    select
      ...
      ts_headline('english', coalesce(h ->> 'description', ''), q.tsq, 'StartSel=**, StopSel=**') as snippet,
